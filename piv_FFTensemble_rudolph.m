function [xtable, ytable, utable, vtable, typevector,correlation_map] = piv_FFTensemble_rudolph(filepath,framenum,framepart,image_array,bg_img_A,bg_img_B,clahe,highp,intenscap,clahesize,highpsize,wienerwurst,wienerwurstsize,roi_inpt,converted_mask,interrogationarea,step,subpixfinder,passes,int2,int3,int4,mask_auto,imdeform,repeat,do_pad)
...
% modified by MLR to take a cell array of frames rather than a file list
cancel=0;
...
if isempty(image_array) %list with image files was passed
	amount_input_imgs=size(filepath,1);
else
	amount_input_imgs=length(image_array);
end
...
for ensemble_i1=1:2:amount_input_imgs
    ...
    else % image array was passed
        image1 = image_array{ensemble_i1};
        image2 = image_array{ensemble_i1+1};
    ...
    if isempty(converted_mask)
    	converted_mask=zeros(size(image1(:,:,1)),'logical');
    end
	%prepare a matrix for calculating the average mask of all images
	if ensemble_i1==1
        average_mask=zeros(size(converted_mask));

		try
			for ii = 1: size(converted_mask,1)
				average_mask= average_mask + converted_mask;
			end
		catch
			cancel = 1;
